...
class MLPmodel(torch.nn.Module):
    def __init__(self, args, ndatatypes, nreciptypes, nconditions, nuser):
        ...
        self.user_embed = torch.nn.Embedding(self.n_user, self.d_emb)
        self.layer1 = torch.nn.Linear(self.d_emb * 4, self.d_hid)
    ...
    def forward(self, x):
        ...
        user_feats = self.user_embed(x[:, 3])
        x = torch.cat([datafeas, recip_feats, cond_feats, user_feats], dim=-1)
        for n in range(self.nlayers):
